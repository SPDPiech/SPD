...
uint32_t ProblemPrzeplywowy::Carlier(string* NajlepszaKolejnosc)
{
    vector<DaneSchrage> ListaZadan;
    vector<uint32_t> Kolejnosc;
    Kolejnosc.clear();
    ...
    Carlier_rekurencja(0, &Kolejnosc, ListaZadan, &Dane);
    char Pomocniczy[32];
...
uint32_t ProblemPrzeplywowy::Carlier_rekurencja(uint64_t n, vector<uint32_t> *NajlepszaKolejnosc, vector<DaneSchrage> ListaZadan, DaneCarlier* Dane)
{
    vector<uint32_t> Kolejnosc;
    vector<uint32_t> KolejnoscPom;
    uint32_t c = 0;

    Dane->U = SchrageC(ListaZadan, &Kolejnosc);
    ...
    if(Dane->U < Dane->UB)
    {
        ...
        {
            NajlepszaKolejnosc->push_back(Kolejnosc[i]);
        }
    }
    ...
    int jpom = 9999999;
    KolejnoscPom.clear();
    for(uint32_t j = 1; j <= Kolejnosc.size(); j++)
    {
        KolejnoscPom.push_back(Kolejnosc[j-1]);

        if(Dane->U == CzasKoncaCRPQ(ListaZadan, KolejnoscPom) + ListaZadan[Kolejnosc[j-1]].q && j < jpom)
        {
            jpom = j;
        }
    }
    ...
    for(uint32_t j = 1; j <= Kolejnosc.size() && Dane->b != 0; j++)
    {
        suma = 0;
        for(uint32_t s = j; s <= Dane->b; s++)
        {
            suma+=ListaZadan[Kolejnosc[s-1]].p;
        }

        if((Dane->U == ListaZadan[Kolejnosc[j-1]].r + suma + ListaZadan[Kolejnosc[Dane->b-1]].q) && jpom > j)
        {
            jpom = j;
        }
    }
    ...
    for(uint32_t j = Dane->a; j <= Dane->b; j++)
    {
        if(ListaZadan[Kolejnosc[j-1]].q < ListaZadan[Kolejnosc[Dane->b-1]].q && j > jpom)
        {
            jpom = j;
        }
    }
    ...
    uint32_t rK = 99999999;
    uint32_t qK = 99999999;
    uint32_t pK = 0;


    for(uint32_t j = c+1; j <= Dane->b; j++)
    {
        rK = min(rK, ListaZadan[Kolejnosc[j-1]].r);

        pK += ListaZadan[Kolejnosc[j-1]].p;

        qK = min(qK, ListaZadan[Kolejnosc[j-1]].q);

    }

    uint32_t rKzC = 9999999;
    uint32_t qKzC = 9999999;
    uint32_t pKzC = 0;

    for(uint32_t j = c+1; j <= Dane->b; j++)
    {
        rKzC = min(rKzC, ListaZadan[Kolejnosc[j-1]].r);

    }

    for(uint32_t j = c; j <= Dane->b; j++)
    {
        qKzC = min(qKzC, ListaZadan[Kolejnosc[j-1]].q);
        pKzC += ListaZadan[Kolejnosc[j-1]].p;
    }

    uint32_t r_pi = ListaZadan[Kolejnosc[c-1]].r;
    ListaZadan[Kolejnosc[c-1]].r = max(ListaZadan[Kolejnosc[c-1]].r, rK + pK);


    Dane->LB = SchrageZPodzialemC(ListaZadan);
    Dane->LB = max(Dane->LB, rK+pK+qK);
    Dane->LB = max(Dane->LB, rKzC+pKzC+qKzC);

    if(Dane->LB < Dane->UB)
    {
        Carlier_rekurencja(n+1, NajlepszaKolejnosc, ListaZadan, Dane);
    }


    ListaZadan[Kolejnosc[c-1]].r = r_pi;
    ...
    uint32_t q_pi = ListaZadan[Kolejnosc[c-1]].q;
    ListaZadan[Kolejnosc[c-1]].q = max(ListaZadan[Kolejnosc[c-1]].q, qK + pK);
    ...
    ListaZadan[Kolejnosc[c-1]].q = q_pi;
...
uint32_t ProblemPrzeplywowy::SchrageC(vector<DaneSchrage> ListaZadan, vector<uint32_t>* NajlepszaKolejnosc)
{
     vector<DaneSchrage> ListaZadanPom;
    for(size_t i = 0; i < ListaZadan.size(); i++)
    {
        ListaZadanPom.push_back(ListaZadan[i]);
    }
    ...
    for(size_t i = 0; i < ListaZadanPom.size(); i++)
    {
        ListaZadan.push_back(ListaZadanPom[i]);
    }
    ...
    while(!GotoweZadania.empty() || !PosortowanaTablica.empty())
    {
        while(!PosortowanaTablica.empty() && ListaZadan[PosortowanaTablica[0]].r <= t)
        {
            //cout<<"DaneZadan[PosortowanaTablica[0]][0]: "<<DaneZadan[PosortowanaTablica[0]][0]<<endl;
            GotoweZadania.push_back(PosortowanaTablica[0]);
            PosortowanaTablica.erase(PosortowanaTablica.begin());
        ...
        if(GotoweZadania.empty())
        {
            t = ListaZadan[PosortowanaTablica[0]].r;
        ...
        }else
        {
            ...
            for(; Iterator1 != GotoweZadania.end(); Iterator1++)
            {
                if(ListaZadan[*Iterator1].q > MaxWartosc)
                {
                    //cout<<"ite1 :"<<*Iterator1<<endl;
                    MaxWartosc = ListaZadan[*Iterator1].q;
                    NumerZadania = *Iterator1;
                    Iterator2 = Iterator1;
                }
            }
    ...
    for(size_t k = 0; k < Kolejnosc.size(); k++)
    {
        NajlepszaKolejnosc->push_back(Kolejnosc[k]);
    }

    return MaxCzasCRPQ(ListaZadan, Kolejnosc);
}

uint32_t ProblemPrzeplywowy::SchrageZPodzialemC(vector<DaneSchrage> ListaZadan)
{
    vector<DaneSchrage> ListaZadanPom;
    ListaZadanPom.push_back(DaneSchrage(0,0,0,0));
    for(size_t i = 0; i < ListaZadan.size(); i++)
    {
        ListaZadanPom.push_back(ListaZadan[i]);
    }
    ...
    while(!ListaZadan.empty())
    {
        ...
        PosortowanaTablica.insert(PosortowanaTablica.end(), NumerZadania+1);
        ListaZadan.erase(Iterator);
        NumerZadania = 0;
        MinWartosc = 2222222222;
    }
    ...
    while(!GotoweZadania.empty() || !PosortowanaTablica.empty())
    {
        while(!PosortowanaTablica.empty() && ListaZadan[PosortowanaTablica[0]].r <= t)
        {
            ...
            if(ListaZadanPom[*(GotoweZadania.end()-1)].q > ListaZadanPom[l].q)
            {
                ListaZadanPom[l].p = t - ListaZadan[*(GotoweZadania.end()-1)].r;
                t = ListaZadan[*(GotoweZadania.end()-1)].r;

                if(ListaZadanPom[l].p > 0)
                    GotoweZadania.push_back(l);
            }
        ...
        }else
        {
            ...
            l = *Iterator2;
            t += ListaZadanPom[*Iterator2].p;
            Cmax =  max(Cmax, t + ListaZadan[*Iterator2].q);
            GotoweZadania.erase(Iterator2);
        }
    }
...
uint32_t ProblemPrzeplywowy::CzasKoncaCRPQ(vector<DaneSchrage> ListaZadan, vector<uint32_t> Kolejnosc)
{
    vector<uint32_t>S;
    uint32_t j = 0;
    vector<uint32_t> Cmax;
    ...
    for(; j < Kolejnosc.size(); j++)
    {
        if(j == 0)
        {
            S.push_back(ListaZadan[Kolejnosc[j]].r);


        }else
        {
            S.push_back(max(ListaZadan[Kolejnosc[j]].r, S[j-1]+ListaZadan[Kolejnosc[j-1]].p));
        }

        Cmax.push_back(S[j] + ListaZadan[Kolejnosc[j]].p);
    }
...
uint32_t ProblemPrzeplywowy::MaxCzasCRPQ(vector<DaneSchrage> ListaZadan, vector<uint32_t> Kolejnosc)
{
    vector<uint32_t>S;
    uint32_t j = 0;
    vector<uint32_t> Cmax;
    ...
    for(size_t i = 0; i < Cmax.size(); i++)
    {
        if((Cmax[i] + ListaZadan[Kolejnosc[i]].q) > MaxWartosc)
            MaxWartosc = Cmax[i] + ListaZadan[Kolejnosc[i]].q;
    }
